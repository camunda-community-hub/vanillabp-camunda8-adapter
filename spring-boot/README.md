...
By default, the adapter reports only the `message` of any exception thrown in the `@WorkflowTask`.
If the `message` is `null`, the exception class name is reported as the message instead. 

To report the entire stack trace as the error message, use the `report-errors-as-stack-trace` flag:
...
vanillabp:
  workflow-modules:
    Demo:
      adapters:
        camunda8:
          report-errors-as-stack-trace: true
```

> [!NOTE]  
> Camunda 8 cuts of any error message after 4000 characters.
> In large exceptions, particularly those generated by Spring Boot, this may cut off the causing exceptions.
...
## Retries
To enable retries in service tasks, simply specify the corresponding Zeebe BPMN attribute:
```xml
<bpmn:serviceTask id="ExampleActivity" name="Transfer money">
  <bpmn:extensionElements>
    <zeebe:taskDefinition type="transferMoney" retries="3" />
  </bpmn:extensionElements>
</bpmn:serviceTask>
```
...
The format of the backoff duration is an ISO-8601 duration that is parseable as `java.time.Duration`.
